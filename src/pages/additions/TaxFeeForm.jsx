...
export default function TaxFeeForm({ mode = 'create', initial, onSubmit, titleOverride }) {
    ...
    const validate = () => {
        ...
        if (isPercent) {
            ...
            if (!form.currency) return 'Currency is required for FIXED tax/fee';
        }
        return null;
    };
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            setError(e?.response?.data?.error || e.message || (mode === 'edit' ? 'Error saving tax/fee' : 'Error creating tax/fee'));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div>
            <h1 style={{ marginTop: 0 }}>{titleOverride || (mode === 'edit' ? 'Edit Tax/Fee' : 'New Tax/Fee')}</h1>
